-- 20260812000002_ndpr_consent.sql

ALTER TABLE public.tenants 
ADD COLUMN IF NOT EXISTS ndpr_consent_granted BOOLEAN NOT NULL DEFAULT FALSE,
ADD COLUMN IF NOT EXISTS ndpr_consent_date TIMESTAMPTZ;

-- RPC for the tenant to accept NDPR consent
CREATE OR REPLACE FUNCTION public.accept_ndpr_consent(_session_token TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET ndpr_consent_granted = TRUE,
      ndpr_consent_date = now(),
      updated_at = now()
  WHERE id = _s.tenant_id;

  RETURN TRUE;
END;
$$;

-- Create an upgraded verify_school_pin that includes the consent flag
