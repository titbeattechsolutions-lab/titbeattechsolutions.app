-- 20260817000002_add_plan_tier_to_session.sql
-- Fixes Bug 1: Add plan_tier (billing tier: micro/starter/growth/enterprise)
-- to the verify_school_pin_v4 RPC so the frontend can enforce correct limits.

CREATE OR REPLACE FUNCTION public.verify_school_pin_v4(_tenant_code TEXT, _pin TEXT)
RETURNS TABLE(
  session_token TEXT,
  tenant_id UUID,
  school_name TEXT,
...
BEGIN
  ...
  IF public._verify_pin_any(_pin, _t.school_pin_hash) THEN
    ...
    -- Fetch plan_tier from billing table (micro/starter/growth/enterprise)
    SELECT s.id INTO _s FROM public.schools s WHERE s.tenant_id = _t.id LIMIT 1;
    SELECT COALESCE(b.plan, 'starter') INTO _tier
    FROM public.billing b
    WHERE b.school_id = _s.id
    LIMIT 1;
    -- Default to 'starter' if no billing row yet
    IF _tier IS NULL THEN _tier := 'starter'; END IF;
